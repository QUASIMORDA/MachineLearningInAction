...
def classifyPerson():
    ...
    ffMiles = float(input("frequenct flier miles earned per year?"))
...
def handwritingClassTest():
    hwLabels = []
    trainingFileList = listdir('data/kNN/digits/trainingDigits')
    ...
    testFileList = listdir('data/kNN/digits/testDigits')
    errorCount = 0.0
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]
        classNumStr = int(fileStr.split('_')[0])
        vectorUnderTest = img2vector('data/kNN/digits/testDigits/%s' % fileNameStr)
